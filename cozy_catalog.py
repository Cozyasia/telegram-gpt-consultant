# -*- coding: utf-8 -*-
"""Independent Cozy Asia property catalog for one Telegram channel + one Sheet."""
...
def extract_lot_id(text: str) -> str:
    ...
    lines = [x.strip() for x in norm.splitlines()[:5] if x.strip()]
    head = "\n".join(lines)
    for pat in [r"(?i)(?:лот|lot)\s*(?:№|#|no\.?)?\s*[:\-]?\s*(\d{3,7})", r"(?:№|#)\s*(\d{3,7})"]:
        m = re.search(pat, head)
        if m:
            return m.group(1).lstrip("0") or "0"
    if lines:
        original_first = (raw.splitlines() or [""])[0]
        if "\u20e3" in original_first or "➖" in original_first:
            groups = re.findall(r"(?<!\d)(\d{3,7})(?!\d)", lines[0])
            if groups:
...
def _fallback(text: str) -> Dict[str,str]:
    ...
    if "вилла" in low or "villa" in low: d["тип"]="вилла"
    elif "дом" in low or "house" in low: d["тип"]="дом"
    elif any(x in low for x in ["апартамент","квартир","condo","apartment"]): d["тип"]="апартаменты"
    m = re.search(r"(?i)(\d+)\s*(?:спальн|bedroom|br\b)", text or "")
    if m: d["спальни"] = m.group(1)
    if "бассейн" in low or "pool" in low: d["бассейн"]="yes"
    return d
...
def _extract(text: str) -> Dict[str,str]:
    ...
    try:
        ...
        prompt = """Извлеки факты из объявления Cozy Asia об аренде недвижимости на Самуи. Верни ТОЛЬКО JSON. Ничего не додумывай. Если факта нет — пустая строка или unknown. Не считай контакты Cozy Asia контактом собственника. контакт_собственника заполняй только если явно написано owner/landlord/собственник/хозяин. Числовые цены — только цифры. Ключи: lot_id, тип, район, спальни, ванные, бассейн, тип_бассейна, цена_месяц_thb, цена_сутки_thb, депозит_thb, комиссия_thb, до_моря_м, доступность, питомцы, электричество, вода, контакт_собственника, описание, confidence, needs_review."""
        resp = client.chat.completions.create(model=OPENAI_MODEL, messages=[{"role":"system","content":prompt},{"role":"user","content":text}], response_format={"type":"json_object"}, temperature=0, max_tokens=900)
...
def upsert_listing(*, text: str, message_id: str, telegram_url: str, published_at: str="", force: bool=False):
    with _lock:
        ...
        rec = {h:"" for h in HEADERS}; rec.update(parsed)
        rec.update({"telegram_message_id":str(message_id),"telegram_url":telegram_url,"published_at":published_at,"status":"active","исходный_текст":text or "","extracted_at":_now()})
        if current:
            old = current[1]
            if old.get("контакт_собственника","").strip(): rec["контакт_собственника"] = old["контакт_собственника"].strip()
            if old.get("status","").strip(): rec["status"] = old["status"].strip()
            rn = current[0]
            ws.update(f"A{rn}", [[rec.get(h,"") for h in headers]], value_input_option="USER_ENTERED")
            action="updated"
        else:
            ws.append_row([rec.get(h,"") for h in headers], value_input_option="USER_ENTERED")
            action="inserted"
        return {"action":action,"lot_id":rec.get("lot_id","")}


def _public_posts(channel: str):
...
def import_public_channel_latest(limit: int=20, force: bool=False):
    limit=max(1,min(int(limit or 20),50))
    ws=ensure_lots_sheet(); existing,_=_existing(ws)
    posts=_public_posts(CATALOG_CHANNEL)[-limit:]
    stats={"channel":CATALOG_CHANNEL,"inspected":len(posts),"listing_candidates":0,"inserted":0,"updated":0,"skipped":0,"errors":0,"lots":[]}
    for p in posts:
        lot=extract_lot_id(p["text"])
        if not _is_listing(p["text"],lot): continue
        stats["listing_candidates"]+=1
        try:
            old=existing.get(p["message_id"])
            if old and not force and old[1].get("исходный_текст","")==p["text"]:
                stats["skipped"]+=1
                if old[1].get("lot_id"): stats["lots"].append(old[1]["lot_id"])
                continue
            res=upsert_listing(text=p["text"],message_id=p["message_id"],telegram_url=p["telegram_url"],published_at=p["published_at"],force=force)
            stats[res["action"]]+=1
            if res.get("lot_id"): stats["lots"].append(res["lot_id"])
            existing,_=_existing(ws)
        except Exception:
            stats["errors"]+=1; log.exception("Import failed for %s", p["message_id"])
    stats["lots"]=list(dict.fromkeys(stats["lots"]))
    return stats


def catalog_status():
    ws=ensure_lots_sheet(); rows=ws.get_all_values()
    if len(rows)<=1: return {"rows":0,"last":[]}
    headers=rows[0]; data=[]
    for row in rows[1:]:
        row=row+[""]*max(0,len(headers)-len(row)); data.append(dict(zip(headers,row)))
...
async def cmd_catalog_import(update, context):
    ...
    try:
        ...
        await update.effective_message.reply_text(f"Каталог обновлён. Проверено: {s['inspected']}; объявлений: {s['listing_candidates']}; добавлено: {s['inserted']}; обновлено: {s['updated']}; без изменений: {s['skipped']}; ошибок: {s['errors']}. Лоты: {', '.join(s['lots'][:20]) or '—'}")
    except Exception as e:
        log.exception("Manual catalog import failed"); await update.effective_message.reply_text(f"Импорт не выполнен: {type(e).__name__}: {e}")
...
async def catch_catalog_updates(update, context):
    ...
    if not _is_listing(text,lot): return
    published=""
    if getattr(msg,"date",None):
        try: published=msg.date.astimezone(timezone.utc).isoformat(timespec="seconds")
        except Exception: published=str(msg.date)
    try:
        await asyncio.to_thread(upsert_listing,text=text,message_id=str(msg.message_id),telegram_url=f"https://t.me/{CATALOG_CHANNEL}/{msg.message_id}",published_at=published,force=True)
    except Exception:
